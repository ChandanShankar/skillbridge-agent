...
describe('ConversationStore', () => {
  ...
  it('deletes history for a conversation', () => {
    store.setHistory('C1', 'T1', [{ role: 'user', content: 'hello' }]);
    store.deleteHistory('C1', 'T1');
    assert.strictEqual(store.getHistory('C1', 'T1'), null);
  });

  it('keeps only recent messages', () => {
    const shortHistoryStore = new ConversationStore(86400, 1000, 2);
    shortHistoryStore.setHistory('C1', 'T1', [
      { role: 'user', content: '1' },
      { role: 'assistant', content: '2' },
      { role: 'user', content: '3' },
    ]);
    assert.deepStrictEqual(shortHistoryStore.getHistory('C1', 'T1'), [
      { role: 'assistant', content: '2' },
      { role: 'user', content: '3' },
    ]);
  });
