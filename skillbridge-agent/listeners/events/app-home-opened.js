...
export async function handleAppHomeOpened({ client, context, logger }) {
  try {
    ...
    let logoUrl = process.env.SKILLBRIDGE_LOGO_URL || null;
    ...
    if (process.env.SLACK_CLIENT_ID) {
      if (context.userToken) {
        ...
        logoUrl ??= `${base.origin}/assets/skillbridge-agent-logo.png`;
      }
    } else if (process.env.SLACK_REDIRECT_URI) {
      const base = new URL(process.env.SLACK_REDIRECT_URI);
      logoUrl ??= `${base.origin}/assets/skillbridge-agent-logo.png`;
    ...
    const view = buildAppHomeView(installUrl, isConnected, logoUrl);
